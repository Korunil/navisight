...
class HierarchicalNavigableNetworkIndex:
    ...
    def _get_or_init_index(self, superclass_id: int) -> hnswlib.Index:
        if superclass_id not in self.sub_indices:
            ...
            graph_path = os.path.join(self.index_dir, f"hnsw_superclass_{superclass_id}.bin")
            
            # If a serialized HNSW binary file exists on disk, load it directly into RAM
            if os.path.exists(graph_path):
                idx.load_index(graph_path, max_elements=self.capacity)
            else:
                # Fresh initialization if no binary graph exists yet
    ...
    def index_parquet_shard(self, shard_parquet_path: str):
        ...
        for cohort_df in df.partition_by("superclass_id", include_key=True):
            ...
            # Serialize the updated HNSW graph structure to disk immediately
            graph_path = os.path.join(self.index_dir, f"hnsw_superclass_{sc_id}.bin")
            idx.save_index(graph_path)
