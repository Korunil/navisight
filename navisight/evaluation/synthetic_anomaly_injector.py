...
class ManifoldAwareAnomalyPerturbationEngine:
    ...
    def inject_counterfactual_scenario(self, df_vessel_voyage: pl.DataFrame, mode: str = "dead_reckoning", severity: float = 0.85) -> pl.DataFrame:
        ...
        for i, idx in enumerate(range(start_anomaly_idx, end_anomaly_idx)):
            ...
            # ──► ADVERSARIAL UPDATE: INJECT TRUE TELEMETRY DARK PATTERNS ◄──
            # Emulates sudden transponder manipulations and intentional data degradation
            if self.rng.random() < (0.25 * severity):
                # Inject high-frequency positioning jitter
                sim_x_meters[idx] = current_x_m + self.rng.uniform(-350.0, 350.0) * severity
                sim_y_meters[idx] = current_y_m + self.rng.uniform(-350.0, 350.0) * severity
            else:
        ...
        # ──► SENSOR SPOOFING: INJECT COARSE RAW STEP-FUNCTIONS ◄──
        # Simulates intentional velocity overrides often used during cover-up maneuvers
        for noise_idx in range(start_anomaly_idx, end_anomaly_idx):
            if noise_idx % 8 == 0:
                final_lats[noise_idx] += self.rng.choice([-0.004, 0.004]) * severity
