        ...
        with zipfile.ZipFile(zip_path, "r") as archive:
            ...
            for file_idx, csv_member in enumerate(csv_members):
                ...
                for i in range(len(df_tails)):
                    ...
                    # Look up the current trip index offset from cache memory before applying updates
                    existing_snapshot = self.cache.query_vessel_state_read_only(v_id_int)
                    existing_offset = existing_snapshot.trip_id_offset if existing_snapshot is not None else 0
                    ...
                    self.cache.apply_mutation_record(
                        ...
                        {"last_timestamp": int(ts_vals[i]), "last_speed": float(speeds[i]), "last_lon": float(lons[i]), 
                         "last_lat": float(lats[i]), "last_is_static": int(is_static_arr[i]), "trip_id_offset": existing_offset}
                    )
