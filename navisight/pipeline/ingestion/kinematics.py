...
class PiraeusRasterDistanceGrid:
    ...
    def __init__(self):
        self.lat_min, self.lat_max = 37.5, 38.1
        self.lon_min, self.lon_max = 23.1, 24.0
        self.res = 0.001
        self.rows = int((self.lat_max - self.lat_min) / self.res) # 600
        self.cols = int((self.lon_max - self.lon_min) / self.res) # 900
        ...
        for r in range(self.rows):
            ...
            for c in range(self.cols):
                ...
                self.grid[r, c] = 0.0 if (lat_c > 38.05 and lon_c > 23.93) else float(min_d)
...
def compute_kinematic_features(df: pl.DataFrame) -> pl.DataFrame:
    """Applies multi-threaded continuous spatial derivatives in raw, unscaled units."""
    if df.is_empty():
        return df
    ...
    # ──► CRITICAL FIX: UNIFIED TIME-GAP GATING FOR TELEMETRY GAP ISOLATION ◄──
    # If a data flow break exceeds 15 minutes, zero out all derivatives to decouple the gap
    gap_condition = delta_time_col > 900

    raw_dy = (pl.col("lat_raw") - prev_lat) * 60.0
    raw_dx = (pl.col("lon_raw") - prev_lon) * pl.col("lat_raw").radians().cos() * 60.0

    gated_dy = pl.when(gap_condition).then(0.0).otherwise(raw_dy)
    gated_dx = pl.when(gap_condition).then(0.0).otherwise(raw_dx)
    gated_acceleration = pl.when(gap_condition).then(0.0).otherwise(delta_speed / dt_safe)
    gated_turn_rate = pl.when(gap_condition).then(0.0).otherwise(corrected_course_change / dt_safe)
    ...
    geodetic_distance_nm = pl.when(prev_lat.is_null() | gap_condition).then(0.0).otherwise(2 * haversine_a_disp.clip(1e-9, 1.0 - 1e-9).sqrt().arcsin() * 3440.065)
    ...
    return df.with_columns([
        gated_dy.fill_null(0.0).alias("delta_y_nm"),
        gated_dx.fill_null(0.0).alias("delta_x_nm"),
        geodetic_distance_nm.alias("geodetic_displacement_nm"),
        geodetic_distance_nm.alias("geodetic_distance_nm"),
        gated_acceleration.fill_null(0.0).alias("acceleration"),
        corrected_course_change.alias("course_change"),
        dt_log.alias("log_time_diff"),
    ]).with_columns([
        pl.when(gap_condition).then(0.0).otherwise(pl.col("acceleration").diff().over(partition_scope) / dt_safe).fill_null(0.0).alias("jerk"),
        gated_turn_rate.alias("turn_rate"),
        pl.when(pl.col("speed_raw") < 0.5).then(0.0).otherwise(gated_turn_rate / (pl.col("speed_raw") + 1e-4)).clip(-50.0, 50.0).alias("turn_rate_speed_ratio"),
