    ...
    def _stream_vessel_sequences(self, file_list):
        for file_path in file_list:
            ...
            for rg_idx in range(parquet_file.num_row_groups):
                ...
                reliability_arr = df_rg["reliability_score"].to_numpy().astype(np.float32)
                ...
                for split_idx in index_splits:
                    ...
                    sub_rel = reliability_arr[split_idx]
                    ...
                    for f_idx, feat_name in enumerate(self.feature_cols):
                        ...
                        if feat_name in REGISTRY.quality or "voyage_phase" in feat_name or "flag" in feat_name:
                            # Nearest neighbor lookup preserves discrete binary flag steps
                            ...
                            try:
                                cs = CubicSpline(sub_ts, feat_slice, extrapolate=True)
                                equalized_features[:, f_idx] = cs(t_uniform).astype(np.float32)
                            except Exception:
                    ...
                    for start in range(0, len(t_uniform) - self.window_size + 1, self.stride):
                        ...
                        # CRITICAL REMOVAL OF PYTORCH MULTI-THREAD MEMORY ACCUMULATION
                        # .copy() severs structural array reference linkages to the large parent row group array
