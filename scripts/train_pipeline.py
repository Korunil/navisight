...
from scripts.compute_global_stats import compute_and_save_global_stats
...
async def main():
    ...
    # Ingestion runs unscaled; we write the base context parameter block directly
    os.makedirs(os.path.dirname(manifest_output_json), exist_ok=True)
    ...
    # Initialize data engines using raw metrics
    ...
    # ── STAGE 1: INGEST AND ENRICH DATA IN RAW PHYSICAL UNITS ──
    ...
    for year in target_years:
        ...
        class WeatherRouterMeshProxy:
            ...
            def interpolate_slice_causal(self, df_slice):
                ...
                slices = []
                for (m_val,), df_m in df_working.group_by(["_m_idx"]):
                    if self.year in self.registry and m_val in self.registry[self.year]:
                        ...
                        slices.append(df_m)
                return pl.concat(slices).drop("_m_idx")
    ...
    # Compile scaling medians and dynamic spatial envelopes precisely from training data
    compute_and_save_global_stats(
        parquet_root=processed_lakehouse_root, 
        weather_root_dir=weather_extracted_dir, 
        output_path=global_stats_json_path, 
        file_list=train_shards
    )
    
    # ── STAGE 3: EXECUTE PRETRAINING LOOP ──
